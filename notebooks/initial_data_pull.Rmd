...
```{r}
cbd_torvik_game_prediction("Duke", "Notre Dame", date = "20261216", location = "N")
```
...
games_sample <- read.csv(here::here("input_data", "original_pulls",
                    paste0("games_", yr, "_march_madness_bracket.csv"))) %>%
...
final_pairs_fixed1 = read.csv(here::here("input_data", "original_pulls","final_pairs_all_fixed.csv")) %>% 
  select(-c(X))
...
# out_path <- here::here("final_pairs_final.csv")
# write.csv(final_pairs_final, out_path, row.names = FALSE)
...
# out_path <- here::here("date_final_all.csv")
# write.csv(date_final_all, out_path, row.names = FALSE)
```
...
final_mod_df1 = mod_df %>% 
  ...
  select(round, team1_name, team2_name, team1_seed, team2_seed, game_id, next_game_id1, next_game_id2, next_game_id3, next_game_id4, next_game_id5, season)
...
final_mod_df2 <- final_mod_df1 %>%
  ...
  rename(seed_tmp = team1_seed) %>%
  rename(team1_seed = team2_seed) %>%
  rename(team2_seed = seed_tmp)

final_mod_df = bind_rows(final_mod_df1, final_mod_df2) %>% 
  select(-c(team2_name, team2_seed))



# out_path <- here::here("final_bracket_df_w_seed.csv")
# write.csv(final_mod_df, out_path, row.names = FALSE)
